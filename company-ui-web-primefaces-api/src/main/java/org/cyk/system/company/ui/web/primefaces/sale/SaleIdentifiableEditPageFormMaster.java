...
public class SaleIdentifiableEditPageFormMaster implements Serializable {
	...
	public static void prepareSalableProduct(FormDetail detail){
		detail.add(SalableProduct.FIELD_PRODUCT).addBreak();
		detail.add(SalableProduct.FIELD_PROPERTIES).addBreak();
	}
	
	public static void prepareSalableProductStore(FormDetail detail){
	...
	public static void prepareSalableProductStoreCollection(FormDetail detail,final String fieldName,Boolean addExistencePeriodFromDate){
		...
		DataTable dataTable = detail.getMaster().instanciateDataTable(SalableProductStoreCollectionItem.class,isCreateOrUpdate ? SalableProductStore.class : null,new Cell.Listener.Adapter.Default(),Boolean.TRUE);
